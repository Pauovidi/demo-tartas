...
export function parseOrderFormat(text: string): OrderFormat | undefined {
  ...
  if (/\b(cajita|caja|petit|pequena|pequeña|pequeno|pequeño|mini|individual)\b/.test(normalized)) {
    return "cajita"
  }
  if (/\b(tarta|mesa|grande|mediana|mediano)\b/.test(normalized)) return "tarta"
  return undefined
}
...
function splitNameCandidate(value: string) {
  ...
  return withoutPhone
    .split(/[,.!?;:]+/)[0]
    ?.split(
      /\b(?:y\s+)?(?:quiero|queria|quería|necesito|busco|para|seria|sería|quisiera|con|mi\s+correo|correo|email|telefono|teléfono|movil|móvil|numero|número|pero|aunque|porque|que|pues)\b/i
    )[0]
    ?.trim() ?? ""
}
